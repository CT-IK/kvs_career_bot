...
- обязательное ознакомление с политикой конфиденциальности и согласием на обработку персональных данных перед началом регистрации;
...
Для листа с вакансиями бот ожидает факультетные колонки:
...
- `vacancies` — вакансии и привязка к факультетам (`ИТиАБД`, `ИОО`, `ФинФак`, `ВШУ`, `НАБ`, `СНиМК`, `МЭО`, `ФЭБ`, `ЮрФак`);
